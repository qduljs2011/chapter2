...
public class DBSCAN {
	...
	private double eps = 1.5; // 半径
	private int threshold = 3; // 密度值
	...
	public void processDBSCAN(String path) {
		try {
			...
			for (int i = 0; i < edgeList.size(); ++i) {
				out.println(
						"核心点 (" + edgeList.get(i).p + " " + edgeList.get(i).q + ") 之间的距离为: " + edgeList.get(i).weight);
			}
			for (int i = 0; i < edgeList.size(); ++i) {
	...
	public static void main(String[] args) {
		...
		dbscan.setInput("F:/javaTest/dbscan.txt");
		dbscan.printOutput("F:/javaTest/result.txt");
	}
}
